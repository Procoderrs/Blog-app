  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-10 bg-purple-50 p-3 md:p-8 rounded-lg shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <input type="text" placeholder="Short description" value={shortDesc} onChange={(e) => setShortDesc(e.target.value)} className="w-full border p-2 rounded" />
        <input type="file" accept="image/*" onChange={handleImage} />
        {preview && <img src={preview} alt="preview" className="w-40 h-40 object-cover rounded mt-2" />}
        <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="w-full border px-3 py-2 rounded">
          <option value="">Select Category</option>
          {categories.map((c) => <option key={c._id} value={c._id}>{c.name}</option>)}
        </select>
        <Editor content={content} onChange={setContent} />
        <button type="submit" disabled={isSubmitting} className="w-full p-3 rounded font-semibold text-white bg-blue-600">
